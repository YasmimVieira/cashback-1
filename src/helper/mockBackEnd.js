    ...
    window.fetch = function (url, opts) {
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                // // get Usuarios
                // if (url.endsWith('/usuarios') && opts.method === 'GET') {
                //     // check for fake auth token in header and return usuarios if valid, this security is implemented server side in a real application
                //     if (opts.headers && opts.headers.Authorization === 'Bearer fake-jwt-token') {
                //         resolve({ ok: true, text: () => Promise.resolve(JSON.stringify(usuarios))});
                //     } else {
                ...
                // // get user by id
                // if (url.match(/\/usuarios\/\d+$/) && opts.method === 'GET') {
                //     // check for fake auth token in header and return user if valid, this security is implemented server side in a real application
                //     if (opts.headers && opts.headers.Authorization === 'Bearer fake-jwt-token') {
                //         // find user by id in usuarios array
